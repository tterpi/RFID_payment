----------------------------------------------------------------
-- RC522 RFID Reader for NodeMCU LUA by Ben Jackson
-- Updated by Nikos Georgousis
-- This is a port of:
...
function blocksIdentical(blockA, blockB)
	for i,v in ipairs(blockA) do
		if not (blockA[i] == blockB[i]) then
			return false
		end
	end
	return true
